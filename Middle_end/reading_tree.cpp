...
{ 
    FILE* fp = fopen(SAVEFILE_NAME_FRONT, "r");
...
Node_t* Read_Node(int* pos_tree_array, char* tree_array)
{
    ...
    else if (strncmp(&tree_array[*pos_tree_array], "nil", 3) == 0)
    {
        *pos_tree_array += 3;
        return NULL;
    }
...
Node_t* Obrabotka_Node(int* pos_tree_array, char* tree_array)
{
    ...
    switch (new_node_type)
    {
        ...
        case OPER_CODE: 
            ...
            break;
        case NAME_CODE: 
            sscanf(&tree_array[*pos_tree_array], "%d%n", &new_node_value_name, &skip);
            node = Make_Node(new_node_type, {.name_ind = new_node_value_name});
            break;
        case BODY_CODE: 
